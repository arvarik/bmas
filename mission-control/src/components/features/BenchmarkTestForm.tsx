...
export function BenchmarkTestForm({ testId }: { testId?: string }) {
  ...
  const validate = () => {
    if (arms.some((arm) => arm.runtime_id === CLASSIC_PREVIEW_CHOICE)) return "The Classic native pair is test only. Inspect its compile preview before qualification.";
  ...
  return (
    <section className="benchmark-form" aria-labelledby="benchmark-form-title">
      ...
      <fieldset className="benchmark-form__section"><legend>Runtime arms</legend>{arms.map((arm, index) => <div className={`benchmark-arm${arm.runtime_id === CLASSIC_PREVIEW_CHOICE ? " benchmark-arm--classic" : ""}`} key={arm.key}><label><span>Arm name</span><input value={arm.name} onChange={(event) => { setArms((current) => current.map((item) => item.key === arm.key ? { ...item, name: event.target.value } : item)); setPreflight(null); }} /></label><label><span>Runtime</span><select aria-label="Runtime" value={arm.runtime_id} onChange={(event) => { setArms((current) => current.map((item) => item.key === arm.key ? { ...item, runtime_id: event.target.value } : item)); setPreflight(null); }}>{runtimes.map((runtime) => <option key={runtime.id} value={runtime.id}>{runtime.label} · contract {runtime.contract_version}</option>)}<option value={CLASSIC_PREVIEW_CHOICE}>Classic native · test only</option></select></label>{arm.runtime_id === CLASSIC_PREVIEW_CHOICE ? <div className="benchmark-arm__config"><ClassicSpecEditor /></div> : <label className="benchmark-arm__config"><span>Configuration JSON</span><textarea rows={3} spellCheck={false} value={arm.configuration} onChange={(event) => { setArms((current) => current.map((item) => item.key === arm.key ? { ...item, configuration: event.target.value } : item)); setPreflight(null); }} /></label>}<button type="button" className="benchmark-icon-button" disabled={arms.length === 1} aria-label={`Remove arm ${index + 1}`} onClick={() => { setArms((current) => current.filter((item) => item.key !== arm.key)); setPreflight(null); }}><Trash2 size={16} /></button></div>)}<ActionButton variant="secondary" onClick={() => { setArms((current) => [...current, { key: crypto.randomUUID(), name: `Arm ${current.length + 1}`, runtime_id: runtimes[0]?.id ?? "classic", configuration: "{}" }]); setPreflight(null); }}><Plus size={15} /> Add arm</ActionButton></fieldset>
      ...
      <div className="benchmark-form__actions"><ActionButton variant="secondary" disabled={arms.some((arm) => arm.runtime_id === CLASSIC_PREVIEW_CHOICE)} loading={pending === "preflight"} onClick={() => void submit("preflight")}>Run preflight</ActionButton><ActionButton disabled={!preflight || arms.some((arm) => arm.runtime_id === CLASSIC_PREVIEW_CHOICE)} loading={pending === "publish"} onClick={() => void submit("publish")}>{testId ? "Publish revision" : "Publish test"}</ActionButton></div>
    </section>
  );
}
