...
> [!IMPORTANT] This design is now *published prior art* — cite it, mine it, keep the name with attribution
> [Zhang, Shi & Wang (2026), arXiv:2605.29313](https://arxiv.org/abs/2605.29313) published *"PatchBoard: Schema-Grounded State Mutation for Reliable and Auditable LLM Multi-Agent Collaboration"* — independently the same design, under the same name: validated JSON-Patch mutations over shared structured state, a deterministic kernel as the only writer, role-scoped write contracts, transactional commits, logged rejections, and replayable transaction logs. Three consequences for this proposal:
>
> 1. **Cite it.** The substrate is no longer a novelty claim — [doc 15 §1](15-novelty-and-research-directions.md#1-honest-framing-prior-art-vs-contribution) and [§6](15-novelty-and-research-directions.md#6-caveats-so-the-showcase-stays-honest) record this. Our contributions are what runs *on* the substrate.
> 2. **Mine it.** Their evaluation empirically confirms this section's bet (84.6% success vs 30.8% LangGraph on ALFWorld; *zero* committed-state contamination under fault injection) and contributes four lessons now folded into our spec: **bounded context views** were one of the two largest ablation effects, and the *smallest* tested view budget won ([doc 03 §4](03-target-architecture.md#4-what-each-turns-agent-payload-looks-like-target)); RFC 6902 **`test` ops as stale-view preconditions** ([doc 04 §3.1](04-blackboard-protocol.md#31-test-ops-explicit-stale-view-preconditions)); a deterministic **circuit policy** that halted 96% of injected no-op/oscillation livelocks ([doc 05 §5](05-control-unit.md#5-cost-governance--safety-rails)); and evidence that **generated task-specific schemas outperform fixed schemas** (noted as an upgrade path in [doc 04 §1](04-blackboard-protocol.md#entry-types)).
> 3. **Keep the name, attributed.** "PatchBoard" in these docs is a deliberate reference to the published architecture, not an independent coinage. Inventing a synonym would only obscure the lineage.
