...
| 14 | [Implementation Runbook (Exact Prompts)](14-implementing-with-cursor-agents.md) | **The literal build log.** 37 numbered steps, each tagged 🆕 new-agent / ♻️ resume / 🧑 you — every prompt to paste, in order. Hard **actor≠critic** separation (implementer vs reviewer/verifier are different runs), GitHub + live-node gates. You only merge + handle escalations. |
