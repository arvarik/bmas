...
- **🆕 NEW AGENT** — open a brand-new Cursor agent/chat and paste the prompt. Do **not** reuse a previous run.
- **♻️ RESUME Step N** — go back to the existing run from Step N and paste the follow-up (only used to fix that run's own PR).
- **🧑 YOU** — a human action (merge a PR, decide go/no-go). No prompt.

**Why new runs each time:** the rule file you create in Step 0 is loaded automatically into *every* new agent, so a fresh run always boots with the full spec + invariants. Fresh context avoids drift and self-review bias. **The reviewer is always a different run than the implementer** — that is the whole safety mechanism.

**Placeholders:** replace `<PR#>` with the PR number the implementer step created (it prints it). Replace nothing else.

**Pace:** run one phase at a time, top to bottom. (Optional parallelism is called out where safe.)
...
1. Create .cursor/rules/blackboard-migration.mdc with EXACTLY this content:
---
...
feat/bb-phase-0 (off feat/true-blackboard). Plan first (Plan mode), then implement:
...
> Optional parallelism: Phase 4 touches `mission-control/` only, so Steps 26–29 may run concurrently with Phases 2–3 (different files). If you'd rather stay strictly linear, just run it here.
...
Every numbered cell is its **own new agent run**, except the 🧑 column (you) and the resume-to-fix follow-ups noted in each "YOU" step.

---

## Appendix A — Why the actor/critic split

The implement step and the review/verify steps are deliberately **different agent runs** because a single agent that both writes and judges its own work drifts toward approving it (self-review bias), and that error compounds across phases. Independent critics — a fresh Reviewer agent (sees only spec + diff), an independent Verifier (objective output from a node), CI, and Bugbot — decide "done" with evidence, not the author's confidence. You are the cheap final gate: because the critics did the real work, your merge click is low-effort and high-trust. Resuming the implementer run to *fix* its own PR is fine; letting it *approve* its own PR is not.
