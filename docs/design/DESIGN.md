...
- **Typing indicator:** For running tasks, a special bottom row shows "{agent} is {verb}..." with a bouncing ellipsis animation.
...
Mission Control uses a **task-centric architecture** — the sidebar shows task history, and the main area shows either a landing page (task input) or a task detail page with tabbed sub-views. This replaces the previous monolithic dashboard grid.
...
Replaced from feature-navigation to **task-history sidebar**.
...
| Action | Feedback |
|:---|:---|
| Submit task | Textarea clears, optimistic navigation to `/task/[id]` with submitted text visible immediately via `PendingTaskContext`. SSE stream connects in the background. |
...
The project uses Next.js App Router with file-based routing. Feature components live alongside their routes as page components, not in a flat `components/` directory.
